...
const MoviesPage = () => {
  ...
  useEffect(() => {
    ...
    async function getMovies() {
      ...
      const url = `https://api.themoviedb.org/3/search/movie?include_adult=false&language=en-US&page=1&query=${queryValue}&language=en-US`;
      const response = await axios.get(url, options);
      setSearchingMovieData(response.data.results);
    }
    getMovies();
  }, [queryValue]);
  return (
